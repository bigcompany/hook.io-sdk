...
module['exports'] = function Hook (client) {
  ...
  self.run = function (opts, cb) {
    ...
    */
    if (opts.streaming) {
      ...
      client.request('/' + opts.owner + "/" + opts.name, { json: data }, function (err, res, body) {
        if (err) {
          return cb(err);
        }
        cb(null, body);
  ...
  self.all = function hookAll (opts, cb) {
    client.request("/" + opts.owner , { json: true, method: "GET" }, function (err, res, body) {
      if (err) {
        return cb(err);
      }
      cb(null, body);
